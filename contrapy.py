...
def check(*contracts):
    ...
    def contractify(obj, funcname=None, name=None):
        ...
        if isinstance(obj, types.FunctionType):
            return build_contract(obj, ValueError(f"failed check {obj.__name__}"))

        if isinstance(obj, (bool, int, float, str, bytes)):
            return build_contract(
                lambda val: val == obj, ValueError(f"{name} should be {obj}")
            )

        return build_contract(lambda val: False, Exception("invalid contract"))
    ...
    contracts = [
        (
            tuple(
                map(
                    lambda name: name if name != "_return" else "return",
    ...
    def _decorator(func):
        ...
        for argname, contract in func.__annotations__.items():
            contracts.append(((argname,), contractify(contract, name=argname)))

        check_before = [
            (args, contract) for args, contract in contracts if "return" not in args
        ]

        check_after = [
            (args, contract) for args, contract in contracts if "return" in args
        ]
